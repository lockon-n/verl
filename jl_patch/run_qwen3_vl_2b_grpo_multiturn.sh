#!/bin/bash
# Qwen3-VL-2B GRPO multi-turn tool training script (Geo3K)
# Usage:
#   bash jl_patch/run_qwen3_vl_2b_grpo_multiturn.sh
# Optional:
#   ENGINE=vllm bash jl_patch/run_qwen3_vl_2b_grpo_multiturn.sh
...
# W&B run name
EXPERIMENT_NAME=${EXPERIMENT_NAME:-"grpo-qwen3-vl-2b-multiturn"}
...
# Geo3K multi-turn parquet directory; expects train.parquet/test.parquet
DATA_DIR=${DATA_DIR:-"/mnt/hdfs/tiktok_aiic/user/junlongli/data/geo3k_multiturn_w_tool"}
# HF model path (local)
MODEL_DIR=${MODEL_DIR:-"hdfs://harunava/home/byte_malia_gcp_aiic/user/junlongli/models/Qwen/Qwen3-VL-2B-Instruct"}
# Root directory for checkpoints and extra logs
CHECKPOINT_DIR=${CHECKPOINT_DIR:-"/mnt/hdfs/tiktok_aiic/user/junlongli/checkpoints/verl_qwen3_vl_2b_multiturn"}
...
# Multi-turn tool config path
TOOL_CONFIG=${TOOL_CONFIG:-"$PROJECT_DIR/examples/sglang_multiturn/config/tool_config/geo3k_tool_config.yaml"}
...
PP=${PP:-1}
# Number of async agent-loop workers
AGENT_WORKERS=${AGENT_WORKERS:-4}

mkdir -p "$CHECKPOINT_DIR" "$VAL_GEN_DIR" "$ROLLOUT_GEN_DIR"
...
echo "Starting GRPO multi-turn training (Qwen3-VL-2B)..."
echo "Engine: $ENGINE"
...
ARGS=(
    # -------- Base config --------
    # Hydra config search path
    --config-path="$CONFIG_PATH"
    # Base trainer config name
    --config-name=ppo_megatron_trainer

    # -------- Algorithm --------
    # Use GRPO advantage estimator
    algorithm.adv_estimator=grpo
    # Do not add KL directly into reward (use explicit KL loss term in actor objective)
    ...
    # VLM image column name in parquet
    data.image_key=images
    # Keep raw chat messages for agent loop
    data.return_raw_chat=True
    # Skip dataset-level precomputed multimodal tensors (agent loop will rebuild)
    data.return_multi_modal_inputs=False

    # -------- Model --------
    # Actor/ref base model path
    actor_rollout_ref.model.path="$MODEL_DIR"

    # -------- Actor optimization --------
    # Actor learning rate
    actor_rollout_ref.actor.optim.lr=1e-6
    # Global mini-batch size for PPO actor update
    actor_rollout_ref.actor.ppo_mini_batch_size=32
    # Per-GPU micro-batch size for PPO actor update
    ...
    # Pipeline model parallel size (training side)
    actor_rollout_ref.actor.megatron.pipeline_model_parallel_size=$PP
    # Tensor model parallel size (training side)
    actor_rollout_ref.actor.megatron.tensor_model_parallel_size=$TP
    # Context parallel size (training side)
    actor_rollout_ref.actor.megatron.context_parallel_size=$CP
    # Enable mbridge integration for VLM training stack
    ...
    # Async rollout mode for higher throughput
    actor_rollout_ref.rollout.mode=async
    # Inference tensor parallel size (rollout side)
    actor_rollout_ref.rollout.tensor_model_parallel_size=$GEN_TP
    # Per-GPU micro-batch size for rollout logprob
    actor_rollout_ref.rollout.log_prob_micro_batch_size_per_gpu=1
    # Per-GPU micro-batch size for ref logprob
    actor_rollout_ref.ref.log_prob_micro_batch_size_per_gpu=1
    # Target inference GPU memory utilization
    actor_rollout_ref.rollout.gpu_memory_utilization=0.6
    # GRPO group size: rollouts per prompt
    actor_rollout_ref.rollout.n=4
    # Disable vLLM multimodal preprocessor cache (stability workaround)
    ...
    # Tool-call/output formatting style
    actor_rollout_ref.rollout.multi_turn.format=hermes
    # Max assistant generation turns per trajectory
    actor_rollout_ref.rollout.multi_turn.max_assistant_turns=5
    # Max user/tool/interaction turns per trajectory
    actor_rollout_ref.rollout.multi_turn.max_user_turns=5
    # Max parallel tool calls within one assistant turn
    actor_rollout_ref.rollout.multi_turn.max_parallel_calls=1
    # Tool registry config file
    ...
    # Activation recompute granularity
    +actor_rollout_ref.actor.megatron.override_transformer_config.recompute_granularity=full
    # Number of layers to recompute
    +actor_rollout_ref.actor.megatron.override_transformer_config.recompute_num_layers=1

    # -------- Trainer / logging --------
    # Start actor training immediately (no critic warmup)
    ...
    # Local checkpoint/output root directory
...
python3 -m verl.trainer.main_ppo "${ARGS[@]}" "$@" 2>&1 | tee jl_patch/qwen3_vl_2b_grpo_multiturn.log

echo "----------------------------------------"
echo "Training completed! Log saved to jl_patch/qwen3_vl_2b_grpo_multiturn.log"
